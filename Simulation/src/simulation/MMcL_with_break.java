...
public class MMcL_with_break {
    ...
    public static void main(String[] args) {
        ...
        double mean_service_time=80;        
        int number_of_customers=1000000;
        int queue_capacity = 10;
        double breakdown = 0.01;
        
        double repair_wait_time;
        ...
        while(Global_Timer.current_time<=total_simulation_time){
            ...
            if(Global_Timer.current_time==0){ //gets the first customer in the customer pool into the server1
                ...
                if(!isBrokeDown(breakdown)){
                    repair_wait_time = repair_time();
                    server1.event_start_time = 0 + repair_wait_time;
                }
                else{
                ...
                if(waiting_queue.isEmpty()){
                    ...
                    if(!server1.isBusy()){
                        ...
                        if(!isBrokeDown(breakdown)){
                            ...
                            repair_wait_time = repair_time();
                            server1.event_start_time = current_customer.arrival_time + repair_wait_time;
                        ...
                        if(!isBrokeDown(breakdown)){
                            repair_wait_time = repair_time();
                            server2.event_start_time = current_customer.arrival_time + repair_wait_time;
                        }
                        else{
                    ...
                    if(!server1.isBusy()){
                        ...
                        if(!isBrokeDown(breakdown)){
                            ...
                            repair_wait_time = repair_time();
                            server1.event_start_time = Global_Timer.current_time + repair_wait_time;
                            total_waiting_time += Global_Timer.current_time + repair_wait_time - current_customer.arrival_time;
                        ...
                        if(!isBrokeDown(breakdown)){
                            ...
                            repair_wait_time = repair_time();
                            server2.event_start_time = Global_Timer.current_time + repair_wait_time;
                            total_waiting_time += Global_Timer.current_time + repair_wait_time - current_customer.arrival_time;
    ...
    public static boolean isBrokeDown(double breakdown){
        
        int rand;
        rand = ThreadLocalRandom.current().nextInt(1, 10001);
        if(rand <= breakdown*10000) return true;
        else return false;
    }
    public static double repair_time(){
        
        int rand;
        rand = ThreadLocalRandom.current().nextInt(1, 4);
        if(rand == 1) return 2;
        else if (rand == 2) return 1;
        else if (rand == 3) return 0.5;
        else if (rand == 4) return 0.1;
        else return 0.01; 
    }
